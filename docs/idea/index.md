...
| --- | --- | --- | --- | --- |
| `all` | [all.md](./all.md) | 2026-05-16 KST | 6 | 공통 모듈 채택·크로스 플랫폼(KMP/iOS/Rust) 빌드 게이트·릴리스 리스크를 함께 낮추는 허브 |
| `all-web-ui` | [all-web-ui.md](./all-web-ui.md) | 2026-05-16 KST | 6 | 공용 토큰·프리미티브 계약과 다운스트림 영향 가시화 허브 |
| `android-support` | [android-support.md](./android-support.md) | 2026-08-22 KST | 6 | Play Console 입력 검증·릴리스 증적과 서브모듈 등록 정합성을 함께 묶는 자동화 |
...
| `youtube` | [youtube.md](./youtube.md) | 2026-08-22 KST | 2 | Bun/uv 이중 워크스페이스 hydration과 glob 패키지 의존성 정합성 허브 |
...
| Path | Status | Why archived |
| --- | --- | --- |
| `toto` | archived 2026-06-04 | Root policy (`AGENTS.md` `/toto` archive policy; `README.md` "Archived child checkouts") excludes `toto` from active submodule, Bun/uv workspace, CodeGraph, codemap-refresh, and idea-gardener targets. Existing entries stay in [toto.md](./toto.md) as historical context; add no new entries unless the user explicitly reactivates the project. |
