...
| --- | --- | --- | --- | --- |
| `all` | [all.md](./all.md) | 2026-08-21 KST | 6 | 공통 모듈 채택·크로스 플랫폼(KMP/iOS/Rust) 빌드 게이트·릴리스 리스크를 함께 낮추는 허브 |
| `all-web-ui` | [all-web-ui.md](./all-web-ui.md) | 2026-08-21 KST | 6 | 공용 토큰·프리미티브 계약과 다운스트림 영향 가시화 허브 |
| `android-support` | [android-support.md](./android-support.md) | 2026-08-21 KST | 5 | Play Console 입력 검증과 릴리스 증적을 묶는 자동화 |
| `Keelim-Knowledge-Vault` | [Keelim-Knowledge-Vault.md](./Keelim-Knowledge-Vault.md) | 2026-08-21 KST | 6 | 워크스페이스 기준선과 문서 스냅샷을 다시 찾게 만드는 허브 |
| `keelim-plugin` | [keelim-plugin.md](./keelim-plugin.md) | 2026-08-21 KST | 6 | 스킬 카탈로그와 설치·회귀 검증을 함께 다루는 개인 플러그인 저장소 |
| `keelim-vercel` | [keelim-vercel.md](./keelim-vercel.md) | 2026-08-21 KST | 6 | 후속 행동 루프와 저장소 계약을 함께 다루는 금융 허브 |
| `rich` | [rich.md](./rich.md) | 2026-08-21 KST | 6 | 운영 복구와 외부 연동 상태를 한곳에서 다루는 관리자 허브 |
| `youtube` | [youtube.md](./youtube.md) | 2026-08-21 KST | 3 | Bun+uv 이중 워크스페이스 멤버십에서 락파일 드리프트와 n8n 자동화 가시성을 다루는 허브 |

## Archived projects (관측 전용, 활성 집계 제외)

| Project | File | Archived | Why archived |
| --- | --- | --- | --- |
| `toto` | [toto.md](./toto.md) | 2026-06-04 (`AGENTS.md` `/toto` archive policy) | 루트 조정 계층에서 archived됨. 로컬 checkout이 남아 있어도 root submodule/workspace/CodeGraph/backlog 대상이 아닌 operator-local 이력 checkout으로만 취급하며, idea gardener 활성 프로젝트 표에서 제외한다. |
...
| `quant` | 원격 저장소가 없는 autonomous local repo(`AGENTS.md` `/quant` policy). 백로그 근거가 될 코드맵/문서 신호가 아직 없어 활성 프로젝트로 승격하지 않는다. |
