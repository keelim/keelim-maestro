...
Why now: AGENTS, CODEMAPS, automation memory, 백로그 노트가 흩어져 있고, 루트 `docs/CODEMAPS/data.md`는 이제 `Keelim-Knowledge-Vault`를 별도 GBrain(`~/brain`) 지식 시스템의 curated import source로 명시하므로, 어떤 노트가 GBrain으로 넘어가는 기준인지도 같은 인덱스에서 바로 찾을 수 있어야 한다.

First slice: 루트 AGENTS/CODEMAPS/idea 인덱스와 자주 바뀌는 운영 규칙, 그리고 GBrain으로 넘어가는 curated 노트 기준을 연결한 단일 인덱스 노트를 만들고, 관련 프로젝트 노트에서 바로 왕복할 수 있게 한다.
