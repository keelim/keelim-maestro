...
- `scripts/check-contract-drift.mjs`와 `scripts/check-ci-release-workflows.mjs`가 이미 `manual-build.yml`에 연결돼 있어, 계약 드리프트와 CI 릴리스 게이트 정적 검사는 해결된 것으로 확인된다(2026-08-04 소스 확인).
