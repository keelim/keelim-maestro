...
### 2026-05-30 - `all` 앱별 Play 트랙 정책과 action 입력 교차 검증

Status: proposed

Why now: `all`은 앱 6종에 대해 각각 `.github/workflows/app_*.yml`을 가지며, `android-support`의 `action.yml`은 `track`, `status`, `userFraction` 같은 Play 정책 입력을 이미 검증한다. 그러나 앱별로 어떤 트랙과 rollout fraction을 써야 하는지 매니페스트가 없어서, 신규 앱 릴리스나 트랙 변경 시 정책 불일치가 실제 업로드 전까지 드러나지 않는다.

First slice: `all`의 앱별 릴리스 workflow 파일에서 `track`·`status`·`userFraction` 값을 수집해 한 JSON 매니페스트로 만들고, `android-support`의 preflight 검증 단계에서 이 매니페스트를 참조해 허용 조합 외의 입력을 즉시 실패로 표시한다.
