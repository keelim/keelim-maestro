...
### 2026-09-07 - 루트 아키텍처 코드맵 커버리지 복원

Status: proposed

Why now: 루트 `AGENTS.md`는 `android-support`를 아홉 개 최상위 child 저장소 중 하나로 명시하지만, `docs/CODEMAPS/architecture.md`의 Workspace Topology 트리와 Subproject Shapes 표에는 `android-support`가 아예 빠져 있다. 새 에이전트나 운영자가 루트 코드맵만 훑고 이 저장소의 존재와 계약(Play Console 업로드 Action)을 놓칠 위험이 있다.

First slice: `docs/CODEMAPS/architecture.md`의 Workspace Topology 트리와 Subproject Shapes 표에 `android-support`(TypeScript GitHub Action / Play Console 릴리스 업로드) 행을 추가해, 다음 코드맵 갱신 때 다른 child 저장소와 동일한 수준으로 노출되게 한다.
