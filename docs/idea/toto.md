...
### 2026-04-25 - CI에서 재현 가능한 클론 게이트 검증 (gitlink pinning 후속)

Status: proposed (gitlink 커밋은 완료)

Why now: gitlink는 이제 루트 인덱스에 커밋돼 있다 (`git ls-files --stage`에서
`toto`가 `5897ef441cb13c550c83a5392097bc46423b3391`로 pin됨, `docs/CODEMAPS/SUBMODULES.md`와
일치). 하지만 이 저장소에는 `.github/workflows`가 아직 없어서, `git submodule update --init toto`
→ `bun run bootstrap` → `bun run verify:toto` 순서가 실제로 그린인지 자동으로 검증하는
장치가 없다. 재현성이 핵심 가치인 프로젝트에서 이 검증 공백이 남은 운영 위험이다.

First slice: `git submodule update --init toto` → `bun run bootstrap` → `bun run verify:toto`를
CI(또는 최소한 pre-push 훅)에서 실행하는 게이트를 추가하고, 그린이 될 때까지는 이 항목을
닫지 않는다.
