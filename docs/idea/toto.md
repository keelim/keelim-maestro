...
Resolution evidence: `git ls-files --stage`가 `160000 5897ef44... toto`를 보여주고,
`docs/CODEMAPS/SUBMODULES.md`와 `docs/CODEMAPS/keelim-maestro.md`도 `toto`를 pinned
`5897ef44` 상태의 등록 submodule 겸 active Bun/uv workspace member로 기록한다. gitlink는
이제 루트 인덱스에 커밋되어 있어 신규 클론에서 `git submodule update --init toto` →
`bun run bootstrap` → `bun run verify:toto` 경로를 그대로 밟을 수 있다. 이 항목은 닫는다.
