...
### 2026-04-25 - CI 서브모듈 부트스트랩 게이트 검증

Status: proposed

Why now: `toto` gitlink(`5897ef44`)는 루트 인덱스에 커밋됐지만, `git submodule update --init toto` → `bun run bootstrap` → `bun run verify:toto` 흐름이 CI 환경에서 실제로 그린인지 명시적으로 검증되지 않았다. 재현성을 핵심 가치로 내세운 프로젝트에서 클론 후 부트스트랩 경로가 무결한지 확인하는 게이트가 없으면 새 기여자나 자동화 환경이 막힌다.

First slice: 신규 클론 + 서브모듈 초기화 → bootstrap → verify:toto 흐름을 CI 워크플로우 또는 로컬 스크립트로 재현하고, 성공 결과를 기록해 재현 가능한 부트스트랩 게이트 완료로 간주한다.
