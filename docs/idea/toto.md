...
Resolution note (2026-05-21): WORKSPACE.md 2026-05-20 기준으로 `toto` gitlink가 루트 인덱스에 `5897ef44`로 커밋되어 있음을 확인했다. CI 단계별 연속 검증은 아래 "루트 CI toto 서브모듈 E2E 부트스트랩 검증" 아이디어로 이어진다.

### 2026-05-21 - 루트 CI toto 서브모듈 E2E 부트스트랩 검증

Status: proposed

Why now: gitlink는 루트 인덱스에 커밋됐지만(`5897ef44`), `git submodule update --init toto` → `bun run bootstrap` → `bun run verify:toto` 전체 체인이 루트 CI에서 자동으로 실행되지 않으면 서브모듈 포인터 갱신이나 toto 소스 변경 시 회귀를 늦게 발견한다. 재현성을 핵심 가치로 내세운 프로젝트에서 이 빈 구간은 gitlink 커밋 다음으로 해소해야 할 운영 위험이다.

First slice: 루트 CI에 toto 서브모듈 init → Python venv bootstrap → `bun run verify:toto` 단계를 추가하고, 포인터 갱신 PR마다 전체 체인이 그린으로 통과하는지 자동으로 확인한다.
