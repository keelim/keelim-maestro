...
### 2026-04-25 - 재현 가능한 클론 CI 게이트 (2026-07-25 재검증: gitlink 커밋 확인됨)

Status: proposed

Why now: 2026-07-25 재검증 결과 `toto` gitlink는 루트 인덱스에 이미 커밋돼 있다(`git ls-files --stage`에서 `160000 5897ef44... toto` 확인 — 이전에 기록했던 "gitlink 미커밋" 상태는 해소됨). 다만 루트에 `.github/` 워크플로가 없어서 `git submodule update --init toto` → `bun run bootstrap` → `bun run verify:toto` 체인을 검증하는 주체가 여전히 사람뿐이고, pin이 바뀌거나 root가 갱신돼도 재현성 깨짐을 자동으로 잡아내지 못한다.

First slice: 위 3단계 체인을 최소 형태의 CI(또는 pre-push 훅)에 등록해, gitlink pin이 바뀔 때마다 자동으로 그린/레드 결과를 남긴다.
