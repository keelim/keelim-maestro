...
Resolution: `git ls-files --stage | grep 160000`와 `docs/CODEMAPS/SUBMODULES.md`(2026-07-12 생성)로 확인한 결과, `toto` gitlink가 `5897ef441cb13c550c83a5392097bc46423b3391`(`main`)로 루트 인덱스에 이미 커밋되어 있다. 신규 클론에서 `git submodule update --init toto` 이후 디렉터리가 비어 있는 문제는 해소됨. 다만 `bun run bootstrap` → `bun run verify:toto` 체인이 실제로 그린인지는 이번 검토에서 서브모듈이 하이드레이트되지 않아 직접 실행 확인하지 못했다 — 하이드레이트된 환경에서 한 번 더 확인 필요.
