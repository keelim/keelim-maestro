...
### 2026-04-25 - 재현 가능한 클론 게이트 CI 통합 (gitlink 커밋 완료)

Status: proposed

Why now: WORKSPACE.md(2026-05-25) 기준 `toto`는 `5897ef44`로 등록 서브모듈에 pinning됐다. gitlink 커밋 자체는 완료됐으나, `git submodule update --init toto` → `bun run bootstrap` → `bun run verify:toto` 전체 경로를 CI에서 자동으로 검증하는 단계가 남아 있다. 시즌 데이터 변동이나 환경 의존성 변경이 있어도 verify가 그린을 유지하는지 지속 보장이 필요하다.

First slice: CI 워크플로우에 `git submodule update --init toto && bun run bootstrap && bun run verify:toto` 단계를 추가하고, seed 재현성과 read-only 계약이 유지되는지 확인한다.
