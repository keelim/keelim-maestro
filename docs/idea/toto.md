...
2026-06-03 WORKSPACE.md 확인 결과 `toto`가 커밋 `5897ef44`로 registered submodule에 pinned됨. `git submodule update --init toto` → `bun run bootstrap` → `bun run verify:toto` 경로가 재현 가능한 상태다.
