...
Why now: `toto`가 `.gitmodules`에 선언되어 있고, 루트 인덱스에 `5897ef44`로 gitlink가 커밋된 것이 WORKSPACE.md에서 확인된다. 그러나 신규 클론 후 `git submodule update --init toto` → `bun run bootstrap` → `bun run verify:toto` 전체 시퀀스가 CI에서 자동으로 실행되고 통과되는지 아직 검증이 필요하다.

First slice: submodule 초기화 → `bun run bootstrap` → `bun run seed` → `bun run verify:toto` 순서가 CI에서 그린으로 완료되면 재현성 기준선 pinning 완료로 간주한다.
