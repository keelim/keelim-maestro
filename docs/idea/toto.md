...
### 2026-04-25 - 재현 가능한 클론 CI 게이트 (gitlink 완료 후)

Status: in-progress

Why now: 루트에 `toto` gitlink(`5897ef44`)가 커밋되어 신규 클론 후 submodule 초기화가 가능해졌다. 남은 작업은 `git submodule update --init toto` → `bun run bootstrap` → `bun run verify:toto` 전체 경로가 CI에서 그린으로 통과하는지 검증하는 것이다.

First slice: CI 파이프라인에 `toto` submodule 초기화 단계를 추가하고 `bun run verify:toto`가 통과하는지 확인한다. seed 재현성과 read-only 계약이 신규 클론에서도 유지되면 완료로 간주한다.
