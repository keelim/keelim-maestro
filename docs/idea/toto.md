...
### 2026-04-25 - 재현 가능한 클론 게이트 (gitlink 커밋 완료, CI 검증 없음)

Status: in-progress

Why now: `git ls-files --stage | grep 160000` 기준으로 `toto` gitlink(`5897ef441cb13c550c83a5392097bc46423b3391`)는 이미 루트 인덱스에 커밋돼 있어 이전에 우려했던 미등록 상태는 해소됐다. 다만 루트에는 `.github/workflows`가 전혀 없어서 `git submodule update --init toto` → `bun run bootstrap` → `bun run verify:toto` 체인이 신규 클론에서 실제로 그린인지 자동으로 확인할 방법이 없다. 재현성을 핵심 가치로 내세운 프로젝트에서 이 검증 공백이 이제 가장 앞선 운영 위험이다.

First slice: 신규 클론을 기준으로 `git submodule update --init toto` → `bun run bootstrap` → `bun run verify:toto` 체인을 실행하는 루트 CI 워크플로를 추가하고, 실패 시 어느 단계에서 끊겼는지 표시한다.
