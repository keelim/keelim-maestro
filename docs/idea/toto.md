...
Why now: gitlink는 이미 루트에 커밋됐다(pinned commit `5897ef44`, WORKSPACE.md 2026-06-11 기준). 남은 작업은 `git submodule update --init toto` → `bun run bootstrap` → `bun run verify:toto` 순서를 CI 파이프라인에서 실제로 검증하는 것이다. 재현성을 핵심 가치로 내세운 프로젝트에서 이 CI 게이트가 없으면 pinning이 명목상으로만 완료된 상태다.

First slice: CI 워크플로우에서 위 세 단계를 연속 실행해 전체 순서가 그린으로 돌아오는 것을 확인하면 pinning 완료로 간주한다.
