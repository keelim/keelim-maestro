...
- 해결됨(2026-07-29): gitlink 미커밋 우려는 `git ls-files --stage | grep 160000`과 `docs/CODEMAPS/SUBMODULES.md`로 재확인한 결과 이미 해소되어 있다 — 루트 인덱스에 `toto`가 `5897ef44...`로 커밋돼 있고 `git submodule update --init toto`로 정상 하이드레이션된다. 아래 관련 아이디어는 이번 리뷰에서 제거한다.
