...
### 2026-04-25 - 재현 가능한 클론 부트스트랩 CI 게이트 (업데이트: 2026-07-10)

Status: proposed (gitlink 커밋 완료, CI 검증 자동화는 아직 없음)

Why now: `git ls-files --stage`로 확인한 결과 `toto`의 gitlink(`5897ef44...`)는 이미 루트 인덱스에 커밋되어 있어 신규 클론에서 디렉터리 부재 문제는 해소됐다. 다만 루트에는 `.github` 워크플로가 없어 `git submodule update --init toto` → `bun run bootstrap` → `bun run verify:toto` 순서가 실제로 그린으로 재현되는지 자동으로 확인하는 수단이 아직 없다. 재현성을 핵심 가치로 내세운 프로젝트에서 "커밋은 됐지만 검증은 수동"인 상태는 다음 pinning 확장의 남은 운영 리스크다.

First slice: 위 3단계 부트스트랩 순서를 실행하는 CI 워크플로(또는 최소한 `scripts/`의 검증 스크립트)를 추가하고, 실패 시 어느 단계에서 끊겼는지 표시한다.
