...
Why now: `toto`가 `.gitmodules`에 선언돼 있으며, 2026-06-28 기준 SUBMODULES.md는 커밋 `5897ef44`에 pinned된 상태를 기록하고 있다. gitlink 등록 자체는 완료된 것으로 보이나, CI에서 전체 재현 경로(`git submodule update --init toto` → `bun run bootstrap` → `bun run verify:toto`)가 그린으로 통과하는지 아직 명시적 증적이 없다.

First slice: `git submodule update --init toto` → `bun run bootstrap` → `bun run verify:toto` 순서를 CI에서 실행해 그린 결과를 pinning 완료 증적으로 남긴다. 이후에는 uv workspace의 `kbo-dashboard` 패키지 테스트(`uv run --package kbo-dashboard --group dev pytest toto/tests`)도 같은 CI 흐름에 포함한다.
