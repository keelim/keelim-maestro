...
### 2026-04-25 - gitlink 커밋 확인 및 재현 가능한 클론 CI 게이트

Status: proposed (gitlink 커밋은 2026-07-01 확인 완료, 후속 항목으로 범위 축소)

Why now: `toto`는 `.gitmodules`에 선언돼 있었고, 2026-07-01 기준 `git ls-files --stage`와 `docs/CODEMAPS/SUBMODULES.md`로 gitlink가 루트 인덱스에 커밋되어 있음을 확인했다(pinned `5897ef44`). 남은 위험은 이 pinning이 CI에서 실제 재현 가능한 클론으로 계속 검증되는가이다.

First slice: `git submodule update --init toto` → `bun run bootstrap` → `bun run verify:toto` 순서를 CI 잡으로 등록하고, 신규 클론에서 이 시퀀스가 정기적으로 그린을 유지하는지 재검증한다.
