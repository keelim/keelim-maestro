...
### 2026-05-13 - 시즌 픽스처 교체 절차서

Status: proposed

Why now: `toto`는 로컬 CSV 픽스처를 `bun run seed`로 주입하는 구조라서, 시즌이 바뀌거나 KBO 일정이 갱신될 때 어떤 파일을 교체하고 어떤 검증을 다시 통과시켜야 하는지 절차가 없으면 시즌 전환 시 스냅샷 매니페스트와 seed 결과가 어긋날 위험이 크다.

First slice: 시즌 교체 대상 픽스처 파일 목록, 교체 후 실행해야 할 `bun run bootstrap` → `bun run seed` → `bun run verify` 순서, 그리고 스냅샷 매니페스트 체크섬 재비교 결과를 한 장의 절차서로 정리하고 시즌 전환 PR 체크리스트에 포함한다.
