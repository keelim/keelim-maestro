...
### 2026-04-25 - 서브모듈 클론 체인 CI 검증 게이트

Status: proposed

Why now: gitlink는 루트 인덱스에 커밋됐다(5897ef44). 남은 작업은 `git submodule update --init toto` → `bun run bootstrap` → `bun run verify:toto` 전체 경로가 신규 클론 환경에서 실제로 통과되는지 CI로 확인하는 것이다. 재현성이 핵심 가치인 프로젝트에서 이 경로가 검증되지 않으면 새 기여자나 자동화 환경에서 조용히 깨질 수 있다.

First slice: CI 워크플로에 서브모듈 초기화 → bootstrap → verify 시퀀스를 별도 스텝으로 추가해 신규 클론 시나리오를 재현하고, 실패 시 명시적 오류가 나도록 `bun run verify:toto` 종료 코드를 확인한다.
