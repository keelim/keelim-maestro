...
Progress (2026-05-18): 루트 인덱스에 gitlink `5897ef44`가 커밋돼 있음이 확인됐다 (`git ls-files --stage | grep 160000`). 남은 단계는 CI에서 `git submodule update --init toto` → `bun run bootstrap` → `bun run verify:toto` 전 구간이 그린으로 통과하는 것이다.
