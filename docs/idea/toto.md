...
### 2026-06-06 - 서브모듈 핀 최신화 추적

Status: proposed

Why now: `toto`의 gitlink가 루트 인덱스에 `5897ef44`로 커밋돼 submodule pinning이 완료됐다. 이제 운영 위험은 "gitlink 누락"에서 "pin이 upstream `main`과 너무 많이 뒤처지는 시점에 데이터 스키마나 시즌 데이터가 바뀌어 재현성이 조용히 깨지는 것"으로 이동한다.

First slice: pinned commit과 upstream `main`의 커밋 수 차이를 주기적으로 확인하는 스크립트를 루트 스크립트 헬퍼에 추가하고, 일정 기준 이상 뒤처지면 `bun run verify:toto`를 새 pin 후보로 실행해 그린인지 확인하는 안내를 포함한다.
