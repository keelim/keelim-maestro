...
Resolved: `git ls-files --stage | grep 160000`에서 `toto`가 `5897ef441cb13c550c83a5392097bc46423b3391`(main)으로 커밋돼 있고, `docs/CODEMAPS/SUBMODULES.md`·`.reports/codemap-diff.txt`(2026-07-21 스냅샷부터)도 동일 커밋을 pinned로 기록한다. gitlink 등록 자체는 완료된 것으로 간주하되, `bun run verify:toto`가 CI에서 그린으로 도는지는 별도 확인 대상으로 남긴다.
