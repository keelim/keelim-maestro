...
- 2026-07-20: `toto` gitlink는 이제 루트 인덱스에 커밋되어 있고(`git ls-files --stage`에서 `160000 ... toto` 확인, `README.md`도 "registered submodule ... clean vs `origin/main`"로 기록) 신규 클론에서 `git submodule update --init toto`로 정상 복원된다. 아래 "gitlink 커밋 및 재현 가능한 클론 게이트" 아이디어는 이 근거로 해결(resolved)되어 목록에서 제거한다.
