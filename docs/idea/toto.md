...
Progress (2026-06-27): SUBMODULES.md에서 `toto`가 `5897ef44` 커밋으로 등록된 것이 확인됐다. `.gitmodules` 선언과 root 인덱스 커밋이 이루어진 것으로 보인다. 실제 CI에서 `git submodule update --init toto` → `bun run bootstrap` → `bun run verify:toto` 흐름이 그린으로 통과하는지 확인 후 완료로 닫는다.
