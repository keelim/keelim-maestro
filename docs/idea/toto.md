...
Why now: `toto`의 gitlink가 루트 인덱스에 `5897ef44`로 커밋됐고 WORKSPACE.md(2026-06-04)에서 등록 submodule로 확인된다. 남은 과제는 신규 클론 → `git submodule update --init toto` → `bun run bootstrap` → `bun run verify:toto` 전체 시퀀스가 CI에서 그린으로 통과되는지 확인하는 것이다.

First slice: 루트 CI에서 toto submodule 초기화 → 부트스트랩 → verify 시퀀스를 한 번 돌리고, toto 디렉터리 부재 시 에러가 사람이 읽을 수 있는 메시지로 나오는지까지 확인한다.
