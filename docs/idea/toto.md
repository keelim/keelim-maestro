...
### 2026-04-25 - 루트 등재 및 재현 가능한 클론 게이트

Status: proposed

Why now: `toto`는 현재 루트 `.gitmodules`에도, `docs/CODEMAPS/SUBMODULES.md`의 Autonomous Local Repos 표에도 등재돼 있지 않다. 신규 클론 시 디렉터리 자체가 없어 `bun run dev:toto`·`bun run verify:toto`를 실행할 수 없고, 워크스페이스 문서 어디에서도 이 저장소의 존재를 찾을 방법이 없다. 재현성을 핵심 가치로 내세운 프로젝트에서 이 이중 공백(서브모듈 미등록 + 코드맵 미등재)은 가장 먼저 해소해야 할 운영 위험이다.

First slice: 안정 커밋을 골라 gitlink를 루트 인덱스에 커밋하거나(서브모듈 전환), 그 전까지는 최소한 `docs/CODEMAPS/SUBMODULES.md`의 Autonomous Local Repos 표에 `toto`를 추가해 존재를 문서화한다. `git submodule update --init toto` → `bun run bootstrap` → `bun run verify:toto` 순서가 CI에서 그린으로 돌아오면 pinning 완료로 간주한다.
