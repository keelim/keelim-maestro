...
135. video-pipeline mode receipts. Evidence: `keelim-plugin` commit `6391b62` added `skills/easy-release-note-video-pipeline/SKILL.md`, `agents/openai.yaml`, catalog entries, and passed `scripts/check.sh` plus `scripts/verify-skills.sh --require-codex-meta`; related SkillOpt review session `019fc4ec-cc64-7b21-866a-1c1985d30ab0` still marked `codex-insights` and `easy-release-note-video-pipeline` as `missing-run`. Improvement: Product Design handoffs that touch media production should require `mode=plan|produce|upload-draft|publish`, `platform`, `language`, `visibility`, `AIDisclosure`, `checksResult`, and `finalUrl` fields so a draft upload cannot be mistaken for public publishing.
